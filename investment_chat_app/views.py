            ...
            if metadata and 'source' in metadata:
                ...
                if filename not in unique_files:
                    ...
                    file_details[filename] = {
                        ...
                        'total_pages': max([m.get('page', 0) for m in all_docs.get('metadatas', []) 
...
def load_documents_to_chromadb():
    ...
    try:
        edgar_dir = os.path.join(settings.BASE_DIR, 'investment_chat_app', 'edgar_files')
        ...
        for filename in files_to_process:
            ...
            for chunk in process_pdf_in_batches(file_path, filename):
                ...
                if len(chunk_batch['ids']) >= batch_size:
                    try:
                        ...
                        logger.info(f"Added batch of {batch_size} chunks from {filename}")
            ...
            if chunk_batch['ids']:
                try:
                    ...
                    logger.info(f"Added final batch of {len(chunk_batch['ids'])} chunks from {filename}")
                except Exception as e:
...
def get_relevant_context(query, n_results=8):
    ...
    try:
        ...
        if any(keyword in query.lower() for keyword in ['what documents', 'which reports', 'available files', 'what files']):
        ...
        for i, doc in enumerate(results['documents'][0]):
            ...
            process_date = results['metadatas'][0][i].get('processed_date', 'Unknown date')
            contexts.append(f"From {source} (Page {page}, Processed: {process_date}):\n{doc}\n")
